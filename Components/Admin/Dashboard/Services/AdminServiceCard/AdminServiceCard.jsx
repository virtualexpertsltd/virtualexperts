...
const AdminServiceCard = ({
	...
	const deleteService = (id) => {
		fetch(`https://virtualexperts.net/api/servicesCard/delete/${id}`, {
			method: "DELETE",
		})
			.then((res) => res.json())
			.then((result) => {
	...
	const onSubmitEdit = (data) => {
		...
		if (file === null) {
			fetch("https://virtualexperts.net/api/servicesCard/update", {
				method: "PUT",
          ...
          {servicesCard.top50Reviewers && (
            <p className="fs-14">
              Top 50 Reviewers : ${servicesCard.top50Reviewers} each
					...
					{servicesCard.price && <p className="fs-14">Price : ${servicesCard.price}</p>}
					{servicesCard.crn && <p className="fs-14">Crn : {servicesCard.crn}</p>}
